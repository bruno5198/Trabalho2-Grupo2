...
class adc_func_4_5(object):
    def __init__(self, imageFile, pencilThickness):
        # ------------------------------------------------------------
        # INITIALIZATION
        # ------------------------------------------------------------
        self.original_image = imageFile
        self.thickness = pencilThickness
        self.aux_image = ""
        self.mouseON = True
        self.aux_image2 = ""
        self.last_coordinates = ""
        self.pencil_color = (255, 0, 0)
        self.window_name = 'Advanced Functionality 4 and 5'
        ...
        print('=> Press "q" or "Q" key to abort!')                                                                                                      # Preliminary notes
        print('=> Press "b" or "B" key to change pencil color to Blue!')                                                                                # Preliminary notes.
        print('=> Press "g" or "G" key to change pencil color to Green!')                                                                               # Preliminary notes.
        print('=> Press "r" or "R" key to change pencil color to Red!\n')                                                                               # Preliminary notes.

    def mouse_callback(self, event, x, y, flags, param):
        if self.mouseON:
            if event == cv2.EVENT_LBUTTONDOWN:
                self.last_coordinates = (x, y)  # Circle coordinates.
                cv2.line(self.aux_image, (x, y), (x, y), self.pencil_color, self.thickness)
                cv2.line(self.aux_image2, (x, y), (x, y), self.pencil_color, self.thickness)
            ...
            if event == cv2.EVENT_MOUSEMOVE:
                if self.last_coordinates != '':
                    cv2.line(self.aux_image, (self.last_coordinates[0], self.last_coordinates[1]), (x, y), self.pencil_color, self.thickness)
                    cv2.line(self.aux_image2, (self.last_coordinates[0], self.last_coordinates[1]), (x, y), self.pencil_color, self.thickness)
                    cv2.imshow(self.window_name, self.aux_image2)  # Display the image
                    self.last_coordinates = (x, y)

    def paint_evaluation(self, image, contour_content, num_white_pix_no_num, num_contour):
        ...
        while True:
            ...
            if (key == ord('n')) or (key == ord('N')) or (cv2.getWindowProperty(self.window_name, 1) == -1):            # Check if user pressed the 'q' key or closed the window.
                print(Fore.YELLOW + Style.BRIGHT + '\nPainted image not saved!\n' + Style.RESET_ALL)                    # Test finished message.
                exit()                                                                                                  # Stops the program.
            elif (key == ord('y')) or (key == ord('Y')):                                                                # Check if user pressed the 'w' key.
    ...
    def run(self):
        ...
        for contour in contours:
            ...
            num_white_pix_no_num.append(fill_contour_image_white_pix)                                                   # Get the number of white pixels of filled contour.
        ...
        while True:
            ...
            if (key == ord('q')) or (key == ord('Q')) or (cv2.getWindowProperty(self.window_name, 1) == -1):            # Check if user pressed the 'q' key or closed the window.
                ...
                self.paint_evaluation(image, contour_content, num_white_pix_no_num, num_contour)                        # Call "paint_evaluation" function to evaluate paint accuracy.
                exit()                                                                                                  # Stops the program.
...
def main():
    ...
    if args.get('advancedFunctionality4') is True:                                                                      # Check if user wants to run advanced functionality 4 and 5.
        class_report = adc_func_4_5(args.get('imageFile'), args.get('pencilThickness'))                                 # Call class "adc_func_4_5()".
